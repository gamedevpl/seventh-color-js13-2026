...
let paused=false,t=0,blast=-99,last=performance.now();pause.onclick=()=>{paused=!paused;pause.textContent=paused?'Wznów':'Pauza'};
document.querySelector('#burst').onclick=()=>blast=t;
...
for(let i=0;i<2;i++){gl.viewport(i*640,0,640,640);const vertices=fields[i](dt,t,e,r,wake.checked,+gain.value,blast);updateMesh(meshes[i],vertices,vertices.length);drawMesh(meshes[i],IDENT);}
