...
let glitterTick=0,time=0,strength=8,eye,camR,player={x:0,z:0},units=[],BOOMS=[];
const RAINBOW=[[1,.2,.2],[1,.6,.1],[1,1,.3],[.3,1,.5],[.2,.7,1],[.5,.4,1],[.9,.3,1]],now=()=>time,who=()=>player;
${particles}
return (dt,t,e,r,wake,gain,blast)=>{time=t;strength=gain;BOOMS=t-blast<1.6?[{x:0,z:5,pw:70,t:t-blast}]:[];eye=e;camR=r;units=wake?[{x:Math.sin(t*.7)*15,z:Math.cos(t*.7)*15,sp:37,st:0,wave:1,r:8}]:[];const n=particleVerts(dt);return PBUF.subarray(0,n)};}`;
